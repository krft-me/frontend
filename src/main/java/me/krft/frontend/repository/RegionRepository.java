...
public interface RegionRepository extends ReactiveCrudRepository<Region, Long>, RegionRepositoryInternal {
    @Query("SELECT * FROM region entity WHERE entity.country_id = :id")
    Flux<Region> findByCountry(Long id);

    @Query("SELECT * FROM region entity WHERE entity.country_id IS NULL")
