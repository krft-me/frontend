...
public interface TagRepository extends ReactiveCrudRepository<Tag, Long>, TagRepositoryInternal {
    @Query("SELECT * FROM tag entity WHERE entity.application_user_offer_id = :id")
    Flux<Tag> findByApplicationUserOffer(Long id);

    @Query("SELECT * FROM tag entity WHERE entity.application_user_offer_id IS NULL")
    Flux<Tag> findAllWhereApplicationUserOfferIsNull();
