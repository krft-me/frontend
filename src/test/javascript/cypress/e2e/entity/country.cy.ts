...
describe('Country e2e test', () => {
  ...
  const countrySample = { isoCode: 'Nép' };
  ...
  beforeEach(() => {
    cy.intercept('GET', '/services/krftme-microservice/api/countries+(?*|)').as('entitiesRequest');
    cy.intercept('POST', '/services/krftme-microservice/api/countries').as('postEntityRequest');
    cy.intercept('DELETE', '/services/krftme-microservice/api/countries/*').as('deleteEntityRequest');
  });
  ...
  describe('new Country page', () => {
    ...
    it('should create an instance of Country', () => {
      cy.get(`[data-cy="name"]`).type('Gorgeous').should('have.value', 'Gorgeous');

      cy.get(`[data-cy="isoCode"]`).type('Cub').should('have.value', 'Cub');
